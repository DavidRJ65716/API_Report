...
if (isset($_POST['submit'])){
	...
	fclose($fp);
	$sql="Insert into `documents` (`name`,`path`,`upload_by`,`upload_date`,`status`,`file_type`,`file_size`) values ('$fileName','$path','$uploadBy','$uploadDate','active','$fileType','$fileSize')";
	$dblink->query($sql) or
	...
	fwrite($fp,$content);
	fclose($fp);
	header("Location: https://192.168.56.102/upload.php?msg=success");
}
?>