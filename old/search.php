...
if (isset($_POST['submit'])){
	...
	while ($data=$result->fetch_array(MYSQLI_ASSOC)){
		...
		echo '<td><a href="view.php?fid='.$data['auto_id'].'" onclick="javascript:LogData('.$data['auto_id'].')">View</a></td>';
		echo '</tr>';
		
		//Logs file access
		//$uploadDate=date("Y-m-d H:i:s");
		//$sqllog="INSERT INTO `file_access_log`(`time_stamp`, `employ_id`, `file_id`, `action`) 
		//VALUES ($uploadDate,'user@test.mail',$data[auto_id],'Searched')";
		//$result=$dblinklog-> query($sqllog) or
		die("Something went wrong with $sql<br>".$dblink->error);
	}
	echo '</table>';
}
